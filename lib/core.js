...
(function() {
  ...
  exports.Compiler = Compiler = (function() {
    var e, instance, name, vop, _fn, _fn1, _i, _j, _len, _len1, _ref, _ref1;
    ...
    Compiler.prototype.compile = function(exp) {
      ...
      this.env = env;
      v = this.newconst('v');
      done = this.clamda(v, il["throw"](il["new"](il.symbol('SolverFinish').call(v))));
      exps = [il.assign(il.uservar('_'), il.require('underscore')), il.assign(il.uservar('__slice'), il.attr([], il.symbol('slice'))), il.assign(il.uservar('solve'), il.attr(il.require('./core'), il.symbol('solve'))), il.assign(il.uservar('parser'), il.require('./parser')), il.assign(il.uservar('solvecore'), il.require('./solve')), il.assign(il.uservar('SolverFinish'), il.attr(il.uservar('solvecore'), il.symbol('SolverFinish'))), il.assign(il.uservar('SolverFail'), il.attr(il.uservar('solvecore'), il.symbol('SolverFail'))), il.assign(il.uservar('Solver'), il.attr(il.uservar('solvecore'), il.symbol('Solver'))), il.assign(il.uservar('Trail'), il.attr(il.uservar('solvecore'), il.symbol('Trail'))), il.assign(il.uservar('Var'), il.attr(il.uservar('solvecore'), il.symbol('Var'))), il.assign(il.uservar('DummyVar'), il.attr(il.uservar('solvecore'), il.symbol('DummyVar'))), il.assign(il.uservar('solver'), il["new"](il.symbol('Solver').call())), il.assign(il.uservar('UArray'), il.attr(il.uservar('solvecore'), il.symbol('UArray'))), il.assign(il.uservar('UObject'), il.attr(il.uservar('solvecore'), il.symbol('UObject'))), il.assign(il.uservar('Cons'), il.attr(il.uservar('solvecore'), il.symbol('Cons'))), il.assign(il.state, null), il.assign(il.catches, {}), il.assign(il.trail, il.newTrail), il.assign(il.failcont, done), il.assign(il.cutcont, il.failcont), il.run(il.transparentuserlamda([], this.cont(exp, done)))];
    ...
    Compiler.prototype.alphaName = function(name) {
      return this.env.alphaName(name);
    };

    Compiler.prototype.alphaMacroParam = function(name) {
      return this.env.alphaMacroParam(name);
    };

    Compiler.prototype.uniqueName = function(name, index) {
      return this.env.uniqueName(name, index);
    };
    ...
    Compiler.prototype.getvar = function(v) {
      if (_.isString(v)) {
        return this.env.getvar(il.uservar(v));
      } else {
        return this.env.getvar(v);
      }
    };

    Compiler.prototype.lookup = function(v) {
      if (_.isString(v)) {
        return this.env.lookup(il.uservar(v));
      } else {
        return this.env.lookup(v);
      }
    };
    ...
    Compiler.prototype.specials = {
      ...
      "switch": function(cont, test, clauses, else_) {
        ...
        return this.cont(test, this.clamda(v, il.switch_(v, clauses, this.cont(else_, cont))));
      },
      ...
      "lambda": function() {
        var body, cont, globalCont, p, params;
      ...
      "macro": function() {
        var body, cont, globalCont, p, params, params1;
      ...
      "array": function() {
        ...
        for (i = _i = _ref = length - 1; _i >= 0; i = _i += -1) {
          cont = (function(i, cont) {
            return compiler.cont(args[i], compiler.clamda(xs[i], cont));
          })(i, cont);
        }
        return cont;
      },
      ...
      "cons": function(cont, head, tail) {
        ...
        v = this.newconst('v');
        v2 = this.newconst('v');
        return this.cont(head, this.clamda(v, this.cont(tail, this.clamda(v1, cont.call(il.cons(v, v1))))));
      },
      "funcall": function() {
      ...
      "forin": function() {
        ...
        vari = this.getvar(vari);
        container = il.lamda([], compiler.cont(container, il.idcont)).call();
        body = compiler.expsCont(body, il.idcont);
        return il.begin(il.forin(vari, container), cont.call(null));
      },
      "forof": function() {
        var body, cont, container, vari;
        ...
        container = il.lamda([], compiler.cont(container, il.idcont)).call();
        body = compiler.expsCont(body, il.idcont);
        return il.begin(il.forof(vari, container), cont.call(null));
      },
      "try": function(cont, test, catches, final) {
        var clause, clauses, _i, _len;

        test = il.lamda([], compiler.cont(test, il.idcont)).call();
        clauses = [];
        for (_i = 0, _len = catches.length; _i < _len; _i++) {
          clause = catches[_i];
          caluses.push([this.getvar(caluse[0]), compiler.cont(clause[1], il.idcont)]);
        }
        final = compiler.cont(final, il.idcont);
        return il.begin(il.try_(test, clauses, final), cont.call(null));
      },
      ...
      'unify': function(cont, x, y) {
        ...
        x1 = this.newconst('x');
        y1 = this.newconst('y');
        return this.cont(x, this.clamda(x1, this.cont(y, this.clamda(y1, il.if_(il.unify(x1, y1), cont.call(true), il.throwFail(false))))));
      },
      'notunify': function(cont, x, y) {
        ...
        x1 = this.newconst('x');
        y1 = this.newconst('y');
        return this.cont(x, this.clamda(x1, this.cont(y, this.clamda(y1, il.if_(il.unify(x, y), il.throwFail(false), cont.call(true))))));
      },
      ...
      'orp': function(cont, x, y) {
        ...
        fc = this.newconst('fc');
        return il.begin(il.assign(trail, il.trail), il.assign(state, il.state), il.assign(fc, il.failcont), il.settrail(il.newTrail), il.setfailcont(il.clamda(v, v, il.undotrail(il.trail), il.settrail(trail), il.setstate(state), il.setfailcont(fc), this.cont(y, cont))), il["try"](this.cont(x, cont), e, il.if_(il["instanceof"](e, il.symbol('SolverFail')), il.failcont.call(il.attr(e, il.symbol('value'))), il["throw"](e))));
      },
      ...
      'findall': function(cont, goal, result, template) {
        ...
        if (result == null) {
          ...
          return il.begin(il.assign(result1, []), il.assign(fc, il.failcont), il.setfailcont(il.clamda(v2, il.assign(v1, v2), il.if_(il.unify(this.interlang(result), result1), il.begin(il.setfailcont(fc), cont.call(null)), fc.call(v1)))), this.cont(goal, this.clamda(v, il.assign(v1, v), il.push(result1, il.getvalue(this.interlang(template))), il.throwFail(v1))));
        }
      },
      ...
      'eoi': function(cont) {
        ...
        return il.begin(il.listassign(data, pos, il.state), il.if_(il.ge(pos, il.length(data)), cont.call(true), il.throwFail(false)));
      },
      'boi': function(cont) {
        return il.if_(il.eq(il.index(il.state, 1), 0), cont.call(true), il.throwFail(false));
      },
      'eol': function(cont) {
        var c, pos, text;
      ...
      'step': function(cont, n) {
        ...
        return this.cont(n, this.clamda(v, il.listassign(text, pos, il.state), il.assign(pos1, il.add(pos, v)), il.setstate(il.array(text, pos1)), cont.call(pos1)));
      },
      ...
      'greedymay': function(cont, exp) {
        ...
        v1 = this.newconst('v');
        v2 = this.newconst('v');
        return il.begin(il.assign(fc, il.failcont), il.setfailcont(il.clamda(v, il.assign(v1, v), il.setfailcont(fc), cont.call(v1))), this.cont(exp, this.clamda(v, il.assign(v2, v), il.setfailcont(fc), cont.call(v2))));
      },
      ...
      'parallel': function(cont, x, y, checkFunction) {
        ...
        v = this.newconst('v');
        v1 = this.newconst('v');
        return il.begin(il.assign(state, il.state), this.cont(x, this.clamda(v, v, il.assign(right, il.state), il.setstate(state), this.cont(y, this.clamda(v, il.assign(v1, v), il.if_(il.fun(checkFunction).call(il.state, right), cont.call(v1), il.throwFail(v1)))))));
      },
      ...
      'xxxchar': function(cont, item) {
        ...
        c = this.newconst('c');
        v = this.newconst('v');
        return this.cont(item, this.clamda(v, il.listassign(data, pos, il.state), il.if_(il.gt(pos, il.length(data)), il["return"](il.throwFail(v))), il.begin(il.assign(x, il.deref(v)), il.assign(c, il.index(data, pos)), il.iff(il["instanceof"](x, il.symbol('Var')), il.begin(il.bind(x, c), il.setstate(il.array(data, il.add(pos, 1))), cont.call(il.add(pos, 1))), il.eq(x, c), il.begin(il.setstate(il.array(data, il.add(pos, 1))), cont.call(il.add(pos, 1))), il.attr(il.symbol('_'), il.symbol('isString')).call(x), il.if_(il.eq(il.length(x), 1), il.throwFail(v), il["throw"](il["new"](il.symbol('ExpressionError').call(x)))), il["throw"](il["new"](il.symbol('TypeError').call(x)))))));
      },
    ...
    _ref = ['char', 'followChars', 'notFollowChars', 'charWhen', 'stringWhile', 'stringWhile0', 'literal', 'followLiteral'];
    ...
    _ref1 = ['number', 'quoteString', 'identifier'];
    _fn1 = function(name, vop) {
      return Compiler.prototype.specials[name] = function(cont, item) {
        return cont.call(il[name](il.solver));
      };
    };
    ...
    Compiler.prototype.quasiquote = function(exp, cont) {
      var head, i, length, quasilist, v, _k, _ref2;
      ...
      if (head === "unquote") {
        ...
        for (i = _k = _ref2 = exp.length - 1; _k >= 1; i = _k += -1) {
          e = exp[i];
    ...
    Compiler.prototype.specialsAlpha = {
      ...
      "begin": function() {
        var exps, head, result, _k, _len2;

        head = arguments[0], exps = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
        result = [head];
        for (_k = 0, _len2 = exps.length; _k < _len2; _k++) {
          e = exps[_k];
      ...
      "nonlocal": function() {
        var head, result, vars, _k, _len2;

        head = arguments[0], vars = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
        result = [head];
        for (_k = 0, _len2 = vars.length; _k < _len2; _k++) {
          name = vars[_k];
          result.push(this.alphaName(name));
        }
        return result;
      },
      "variable": function() {
        ...
        result.push((function() {
          ...
          for (_k = 0, _len2 = vars.length; _k < _len2; _k++) {
            name = vars[_k];
            _results.push(this.alphaName(name));
          }
          return _results;
        }).call(this));
        return result;
      ...
      "assign": function(head, left, exp) {
        return [head, this.alpha(left), this.alpha(exp)];
      },
      "uniquevar": function(head, name, index) {
        return [head, this.uniqueName(name, index)];
      },
      ...
      "lambda": function() {
        var body, head, p, params, result, _k, _len2;
        ...
        result.push((function() {
          ...
          for (_k = 0, _len2 = params.length; _k < _len2; _k++) {
            p = params[_k];
            _results.push(this.alphaName(p));
          }
          return _results;
        }).call(this));
      ...
      "macro": function() {
        var bindings, body, head, p, params, result, _k, _l, _len2, _len3;
        ...
        for (_l = 0, _len3 = body.length; _l < _len3; _l++) {
          e = body[_l];
          result.push(this.alpha(e));
        }
        this.popEnv();
      ...
      'block': function() {
        var body, head, label, result, _k, _len2;
