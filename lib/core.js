...
(function() {
  var ADD, AND, ANY, ARRAY, ASSIGN, ATTR, AUGMENTASSIGN, ArgumentError, ArityError, BEGIN, BIND, BITAND, BITNOT, BITOR, BITXOR, BLOCK, BOI, BOL, BREAK, CALLCC, CALLFC, CATCH, CONS, CONTINUE, CUT, CUTABLE, Compiler, CpsEnv, DEC, DECP, DIRECT, DIV, DUMMYVAR, EFFECT, EOI, EOL, EQ, EVAL, EVALARG, Env, Error, FAIL, FINDALL, FOLLOW, FOR, FORIN, FOROF, FUNCALL, GE, GETPARSERCURSOR, GETPARSERDATA, GETPARSERSTATE, GETVALUE, GREEDYANY, GREEDYMAY, GT, IF, IFP, INC, INCP, INDEX, INSTANCEOF, IO, IS, JSFUN, JSFUNCALL, JSTHROW, LAMDA, LAZYANY, LAZYMAY, LE, LEFTPARSERDATA, LENGTH, LIST, LOGICVAR, LSHIFT, LT, MACRO, MACROCALL, MAKEOBJECT, MAY, MOD, MUL, NE, NEG, NEXTCHAR, NONLOCAL, NOT, NOTFOLLOW, NOTP, NOTP2, NOTP3, NOTUNIFY, ONCE, OR, ORP, ORP2, ORP3, OptimizationEnv, PARALLEL, PARSE, PARSEDATA, POP, PURE, PUSH, PUSHP, QUASIQUOTE, QUOTE, REPEAT, RETURN, RSHIFT, SETPARSERCURSOR, SETPARSERDATA, SETPARSERSTATE, SEXPR_HEAD_FIRST, SEXPR_HEAD_LAST, SLICE, STEP, STRING, SUB, SUBPARSERDATA, SUCCEED, SUFFIXDEC, SUFFIXDECP, SUFFIXINC, SUFFIXINCP, SWITCH, THROW, TRY, TypeError, UARRAY, UNIFY, UNIQUECONST, UNIQUEVAR, UNQUOTE, UNQUOTESLICE, UNWINDPROTECT, UOBJECT, VARIABLE, VarLookupError, beautify, compile, fs, hasOwnProperty, il, isArray, isInteger, isString, _ref, _ref1, _ref2, _ref3, _ref4,
    __slice = [].slice,
  ...
  exports.Compiler = Compiler = (function() {
    ...
    Compiler.prototype.alpha = function(exp) {
      var bindings, e, head, left, length, name, p, params, result, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8;
    ...
    Compiler.prototype.cont = function(exp, cont) {
      var anyCont, anyFcont, args, body, c, catchBody, cc, checkFunction, clauses, cleanup, compiler, container, continueCont, continues, data, default1, defaultContinues, defaultExits, exitCont, exits, f, fc, final, fun, head, i, init, k, label, length, length1, length2, length3, list1, list2, name, obj, oldEffect, oldprotect, p, params, params1, parsercursor, pos, result, result1, right, start1, start2, start3, step, temp, temp1, temp2, template, test, text, trail, uobj, v, v1, v2, value, value1, value2, vop, x1, xs, y1, _base, _base1, _base2, _base3, _i, _j, _k, _l, _len, _m, _n, _o, _p, _q, _ref10, _ref11, _ref12, _ref13, _ref14, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
      ...
      switch (head) {
        ...
        case SWITCH:
          ...
          if (length % 2 === 1) {
            default1 = clauses[length - 1];
            clauses = clauses.slice(0, length - 1);
          } else {
            default1 = void 0;
          }
          clauses = (function() {
            ...
            _results = [];
            for (i = _j = 0, _ref3 = length - 1; _j < _ref3; i = _j += 2) {
              _results.push([
                (function() {
                  ...
                  for (_k = 0, _len1 = _ref4.length; _k < _len1; _k++) {
                    value = _ref4[_k];
                    _results1.push(this.cont(value, il.idcont()));
                  }
                  return _results1;
                }).call(this), this.cont(clauses[i + 1], k)
              ]);
            }
            return _results;
          }).call(this);
          default1 = this.cont(default1, k);
          return il.begin(il.assign(k, cont), this.cont(exp[1], il.clamda(v, il["switch"](v, clauses, default1))));
        case JSFUN:
          f = il.jsfun(exp[1]);
        ...
        case ARRAY:
          ...
          cont = cont.call(il.array.apply(il, xs));
        ...
        case UNWINDPROTECT:
          ...
          return result;
        case RETURN:
          return il.begin(il["return"](il.lamda([], this.cont(exp[1], il.idcont())).call()), cont.call(null));
        case JSTHROW:
          return il.begin(il["throw"](il.lamda([], this.cont(exp[1], il.idcont())).call()), cont.call(null));
        case CALLCC:
          v = this.newconst('v');
          k = this.newconst('k');
          return il.begin(il.assign(k, cont), this.cont(exp[1], il.clamda(v, k.call(v.call(k, k)))));
        case CALLFC:
          v = this.newconst('v');
          k = this.newconst('k');
          return il.begin(il.assign(k, cont), this.cont(exp[1], il.clamda(v, k.call(v.call(il.failcont, k)))));
        case LOGICVAR:
        ...
        case PARSEDATA:
          v = this.newconst('v');
          return this.cont(exp[2], il.clamda(v, il.begin(il.setparserdata(v), il.setparsercursor(0), this.cont(exp[1], il.clamda(v, cont.call(v))))));
        case SETPARSERSTATE:
          v = this.newconst('v');
          v1 = this.newconst('v');
    ...
    Compiler.prototype.leftValueCont = function(cont, task, item, exp, op) {
      ...
      assignExpCont = function(item) {
        ...
        switch (task) {
          case ASSIGN:
            return _this.cont(exp, il.clamda(v, cont.call(il.assign(item, v))));
          case AUGMENTASSIGN:
            return _this.cont(exp, il.clamda(v, cont.call(il.assign(item, il[op](item, v)))));
          case INC:
            return cont.call(il.assign(item, il.add(item, 1)));
          case DEC:
            return cont.call(il.assign(item, il.sub(item, 1)));
          case SUFFIXINC:
          ...
          case INCP:
            ...
            return il.begin(il.assign(fc, il.failcont), il.setfailcont(il.clamda(v, il.assign(item, il.sub(item, 1)), fc.call(item))), cont.call(il.assign(item, il.add(item, 1))));
          case DECP:
            fc = _this.newconst('fc');
            return il.begin(il.assign(fc, il.failcont), il.setfailcont(il.clamda(v, il.assign(item, il.add(item, 1)), fc.call(item))), cont.call(il.assign(item, il.sub(item, 1))));
          case SUFFIXINCP:
            fc = _this.newconst('fc');
  ...
  exports.OptimizationEnv = OptimizationEnv = (function(_super) {
    ...
    OptimizationEnv.prototype.lookup = function(vari) {
      ...
      if (bindings.hasOwnProperty(vari)) {
        ...
        if (vari.isConst) {
          outer = this.outer;
          if (outer instanceof OptimizationEnv) {
            return outer.lookup(vari);
          } else {
            return vari;
          }
