...
(function() {
  var ADD, AND, ANY, ARRAY, ASSIGN, AUGMENTASSIGN, ArgumentError, ArityError, BEGIN, BIND, BITAND, BITNOT, BITOR, BITXOR, BLOCK, BOI, BOL, BREAK, CALLCC, CALLFC, CATCH, CONS, CONTINUE, CUT, CUTABLE, Compiler, CpsEnv, DEC, DECP, DIRECT, DIV, DUMMYVAR, EFFECT, EOI, EOL, EQ, EVAL, EVALARG, Env, Error, FAIL, FINDALL, FOLLOW, FOR, FORIN, FOROF, FUNCALL, GE, GETPOS, GETSTATE, GETTEXT, GETVALUE, GREEDYANY, GREEDYMAY, GT, IF, IFP, INC, INCP, INDEX, IO, IS, JSFUN, JSFUNCALL, LAMDA, LAZYANY, LAZYMAY, LE, LEFTTEXT, LIST, LOGICVAR, LSHIFT, LT, MACRO, MACROCALL, MAKEOBJECT, MAY, MOD, MUL, NE, NEG, NEXTCHAR, NONLOCAL, NOT, NOTFOLLOW, NOTP, NOTUNIFY, ONCE, OR, ORP, OptimizationEnv, PARALLEL, PARSE, PARSETEXT, PURE, PUSH, PUSHP, QUASIQUOTE, QUOTE, REPEAT, RSHIFT, SETPOS, SETSTATE, SETTEXT, SEXPR_HEAD_FIRST, SEXPR_HEAD_LAST, STEP, STRING, SUB, SUBTEXT, SUCCEED, SUFFIXDEC, SUFFIXDECP, SUFFIXINC, SUFFIXINCP, SWITCH, THROW, TRY, TypeError, UARRAY, UNIFY, UNIQUECONST, UNIQUEVAR, UNQUOTE, UNQUOTESLICE, UNWINDPROTECT, UOBJECT, VARIABLE, VarLookupError, beautify, compile, fs, hasOwnProperty, il, isArray, isInteger, isString, _ref, _ref1, _ref2, _ref3, _ref4,
    __slice = [].slice,
    ...
    Compiler.prototype.cont = function(exp, cont) {
      ...
      switch (head) {
        ...
        case ORP:
          ...
          fc = this.newconst('fc');
          return il.begin(il.assign(trail, il.trail), il.assign(state, il.state), il.assign(fc, il.failcont), il.settrail(il.newTrail), il.setfailcont(il.clamda(v, v, il.undotrail(il.trail), il.settrail(trail), il.setstate(state), il.setfailcont(fc), this.cont(exp[2], cont))), this.cont(exp[1], cont));
        case IFP:
          v = this.newconst('v');
          fc = this.newconst('fc');
        ...
        case NOTP:
          ...
          state = this.newconst('state');
          fc = this.newconst('fc');
          return il.begin(il.assign(trail, il.trail), il.assign(fc, il.failcont), il.assign(state, il.state), il.settrail(il.newTrail), il.setfailcont(il.clamda(v, il.assign(v1, v), il.undotrail(il.trail), il.settrail(trail), il.setstate(state), il.setfailcont(fc), cont.call(v1))), this.cont(exp[1], fc));
        case REPEAT:
        ...
        case PARSE:
          ...
          return this.cont(exp[2], il.clamda(v, il.assign(oldState, il.state), il.setstate(v), this.cont(exp[1], il.clamda(v, il.assign(v1, v), il.setstate(oldState), cont.call(v1)))));
        case PARSETEXT:
          v = this.newconst('v');
          v1 = this.newconst('v');
          oldState = this.newconst('state');
          return this.cont(exp[2], il.clamda(v, il.begin(il.assign(oldState, il.state), il.setstate(il.array(v, 0)), this.cont(exp[1], il.clamda(v, il.assign(v1, v), il.setstate(oldState), cont.call(v1))))));
        case SETSTATE:
          v = this.newconst('v');
          return this.cont(exp[1], il.clamda(v, il.setstate(v), cont.call(true)));
        case SETTEXT:
          v = this.newconst('v');
          return this.cont(exp[1], il.clamda(v, il.setstate(il.array(v, 0)), cont.call(true)));
        case SETPOS:
          v = this.newconst('v');
          return this.cont(exp[1], il.clamda(v, il.assign(il.index(il.state, 1), v), cont.call(true)));
        case GETSTATE:
        ...
        case STEP:
          ...
          return this.cont(exp[1], il.clamda(v, il.listassign(text, pos, il.state), il.assign(pos1, il.add(pos, v)), il.setstate(il.array(text, pos1)), cont.call(pos1)));
        case LEFTTEXT:
        ...
        case SUBTEXT:
          ...
          return this.cont(exp[1], il.clamda(length1, il.assign(length2, length1), this.cont(exp[2], il.clamda(start1, il.assign(start2, start1), il.listassign(text, pos, il.state), il.begin(il.assign(start3, il.if_(il.ne(start2, null), start2, pos)), il.assign(length3, il.if_(il.ne(length2, null), length2, il.length(text))), cont.call(il.slice(text, start3, il.add(start3, length3))))))));
        case NEXTCHAR:
          text = this.newconst('text');
          pos = this.newconst('pos');
        ...
        case ANY:
          ...
          v = this.newconst('v');
          v1 = this.newconst('v');
          return il.begin(il.assign(anyCont, il.recclamda(v, il.assign(fc, il.failcont), il.assign(trail, il.trail), il.assign(state, il.state), il.settrail(il.newTrail), il.setfailcont(il.clamda(v, il.assign(v1, v), il.undotrail(il.trail), il.settrail(trail), il.setstate(state), il.setfailcont(fc), cont.call(v1))), this.cont(exp[1], anyCont))), anyCont.call(null));
        case LAZYANY:
          fc = this.newconst('fc');
          trail = this.newconst('trail');
          v = this.newconst('v');
          ...
          return il.begin(il.local(trail), il.assign(anyCont, il.recclamda(v, il.nonlocal(trail), il.assign(trail, il.trail), il.settrail(il.newTrail), il.setfailcont(anyFcont), cont.call(null))), il.assign(anyFcont, il.recclamda(v, il.undotrail(il.trail), il.settrail(trail), il.setfailcont(fc), this.cont(exp[1], anyCont))), il.assign(fc, il.failcont), anyCont.call(null));
        case GREEDYANY:
          fc = this.newconst('fc');
          anyCont = this.newconst('anyCont');
          anyCont.isRecursive = true;
        ...
        case PARALLEL:
          checkFunction = exp[3] || (function(state, baseState) {
          ...
          v = this.newconst('v');
          v1 = this.newconst('v');
          return il.begin(il.assign(state, il.state), this.cont(exp[1], il.clamda(v, v, il.assign(right, il.state), il.setstate(state), this.cont(exp[2], il.clamda(v, il.assign(v1, v), il.if_(il.fun(checkFunction).call(il.state, right), cont.call(v1), il.failcont.call(v1)))))));
        case FOLLOW:
          state = this.newconst('state');
          v = this.newconst('v');
          v1 = this.newconst('v');
          state = this.newconst('state');
          return il.begin(il.assign(state, il.state), this.cont(exp[1], il.clamda(v, il.assign(v1, v), il.setstate(state), cont.call(v))));
        case NOTFOLLOW:
          state = this.newconst('state');
          fc = this.newconst('fc');
          v = this.newconst('v');
          v1 = this.newconst('v');
          return il.begin(il.assign(fc, il.failcont), il.assign(state, il.state), il.setfailcont(cont), this.cont(exp[1], il.clamda(v, il.assign(v1, v), il.setstate(state), fc.call(v1))));
        case ADD:
        case SUB:
        case MUL:
        case DIV:
