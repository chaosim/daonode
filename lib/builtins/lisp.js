...
(function() {
  var block, break_, continue_, debug, general, if_fun, iff_fun, macro, not_, runner, solve, special, _,
    __slice = [].slice;
  ...
  runner = function(solver, cont) {
    return function(v) {
      var name, pair, value, vari, _i, _len, _ref, _ref1;

      _ref = solver.vars;
      for (pair = _i = 0, _len = _ref.length; _i < _len; pair = ++_i) {
        name = _ref[pair];
        vari = pair[0];
        value = pair[1];
        vari.binding = value;
      ...
      while (!solver.done) {
        _ref1 = cont(v, solver), cont = _ref1[0], v = _ref1[1], solver = _ref1[2];
      }
      solver.done = false;
      return v;
    };
  ...
  exports.callcc = special(1, 'callcc', function(solver, cont, fun) {
    return function(v, solver) {
      return cont(fun(runner(solver.clone(), cont)), solver);
    };
  });

  exports.callfc = special(1, 'callfc', function(solver, cont, fun) {
    return function(v, solver) {
      return cont(fun(runner(solver.clone(), solver.failcont)), solver);
    };
  });

  exports.callcs = special(1, 'callcs', function(solver, cont, fun) {
    return function(v, solver) {
      return cont(fun(solver.clone(), cont), solver);
    };
  });
