...
(function() {
  var ExpressionError, Trail, TypeError, Var, any1, any2, checkFunction, dao, dqstring, general, greedyany1, greedyany2, greedysome1, greedysome2, lazyany1, lazyany2, lazysome1, lazysome2, lisp, logic, name, numberTimes1Fun, numberTimes2Fun, parallelFun, solve, some1, some2, special, sqstring, times1, times1Fun, times2, times2Fun, _, _ref,
    __slice = [].slice,
  ...
  exports.seplist = function(exp, options) {
    var andp, bind, expectTimes, freep, getvalue, i, ifp, inc, is_, list, n, newVar, one, push, result, sep, sub, succeed, template;
    ...
    if (expectTimes === null) {
      if (result === null) {
        ...
        return andp(bind(result, []), exp, push(result, getvalue(template)), any(andp(sep, exp, push(result, getvalue(template)))));
      }
      ...
      if (result === null) {
        ...
        return andp(bind(result, []), ifp(freep(expectTimes), andp(exp, one(i), push(result, getvalue(template)), any(andp(sep, exp, push(result, getvalue(template)), inc(i))), bind(expectTimes, i)), andp(exp, push(result, getvalue(template)), is_(n, sub(expectTimes, 1)), times(andp(sep, exp, push(result, getvalue(template))), n))));
      }
    }
  };
