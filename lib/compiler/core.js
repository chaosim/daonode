...
(function() {
  ...
  exports.Compiler = Compiler = (function() {
    ...
    Compiler.prototype.compile = function(exp) {
      ...
      v = il.vari('v');
      fromCont = this.cont(exp, il.idcont);
      f = il.assign(il.vari('exports.main'), il.clamda(v, il.assign(il.solver, il["new"](il.symbol('Solver').call())), il.assign(il.state, null), il.assign(il.catches, {}), il.assign(il.trail, il.newTrail), il.assign(il.failcont, il.idcont), il.assign(il.cutcont, il.failcont), fromCont));
      f.refMap = {};
    ...
    Compiler.prototype.specials = {
      ...
      'logicvar': function(cont, name) {
        return cont.call(il.newLogicVar.call(name));
      },
      ...
      'eoi': function(cont) {
        return il.begin(il.listassign(data, pos, il.state), il.if_(il.ge(pos, il.length(data)), cont.call(true), il.failcont.call(v)));
      },
      'boi': function(cont) {
        return il.if_(il.eq(il.index(il.state, 1), 0), cont.call(true), il.failcont.call(v));
      },
      'eol': function(cont) {
        return il.begin(il.listassign(data, pos, il.state), il.if_(il.ge(pos, il.length(data.length), cont.call(true), il.if_(il["in"](text[pos], il.string("\r\n")), cont.call(true), il.failcont.call(v)))));
      },
      'eol': function(cont) {
        return il.begin(il.if_(il.eq(il.index(il.state, 1), 0), cont.call(true), il.if_(il["in"](il.index(il.index(il.state, 0), il.sub(il.index(il.state, 1), 1)), il.string("\r\n")), cont.call(true), il.failcont.call(v))));
      },
      'step': function(cont, n) {
        return this.cont(n, il.clamda(v, il.listassign(text, pos, il.state), il.augassign(pos, v), il.setstate(il.array(text, pos)), cont.call(pos)));
      },
      'lefttext': function(cont) {
        return cont(il.slice(il.index(il.state, 0), il.index(il.state, 1)));
      },
      'subtext': function(solver, cont, length, start) {
        return il.begin(il.listassign(text, pos, il.state), il.assign(start, il.or_(il.ne(start, null), pos)), il.assign(length, il.or_(il.ne(length, null), il.length(pos))), cont.call(il.slice(start, il.add(start, length))));
      },
      ...
      'any': function(cont, exp) {
        return il.begin(il.assign(anyCont, il.clamda(v, il.let_([fc, il.failcont, trail, il.trail, state, il.state], il.settrail(il.newTrail), il.setfailcont(il.clamda(v, il.undotrail, il.settrail(trail), il.setstate(state), il.setfailcont(fc), cont.call(v))), this.cont(exp, anyCont)))), anyCont.call(null));
      },
      'lazyany': function(cont, exp) {
        return il.begin(il.assign(anyCont, il.clamda(v, il.setfailcont, anyFcont), cont.call(v)), il.assign(anyFcont, il.clamda(v, il.setfailcont(fc), this.cont(exp, anyCont))), il.assign(fc, solver.failcont), anyCont.call(v));
      },
      'greedyany': function(cont, exp) {
        return il.begin(il.assign(anyCont, il.clamda(v, this.cont(exp, anyCont))), il.assign(fc, il.failcont, il.setfailcont(il.clamda(v, il.setfailcont(fc), cont.call(v))), anyCont.call(v)));
      },
      ...
      'char': function(cont, item) {
        var v;

        v = il.vari('v');
        return this.cont(item, il.clamda(v, cont.call(il.char(il.solver, v))));
      }
    };
