...
(function() {
  var Apply, Assign, AugmentAssign, Begin, BinaryOperation, CApply, Clamda, Code, Deref, Element, Env, Fun, IO, IdCont, If, InternalLocalVar, InternalNonlocalVar, InternalVar, JSFun, Lamda, ListAssign, LocalDecl, MAX_EXTEND_CODE_SIZE, New, NonlocalDecl, NotImplement, Print, RecursiveClamda, Return, Symbol, TASK_FUNC, TASK_OPTIMIZE, Throw, TopBegin, UnaryOperation, UserLamda, UserLocalVar, UserNonlocalVar, UserVar, Var, VirtualOperation, analyze, appendTasks, applySideEffect, augmentAssign, augmentOperators, begin, binary, boolize, codeSize, core, expressionToCode, expsEffect, hasOwnProperty, il, insertReturn, isAtomic, isDeclaration, isEmpty, isJsified, isStatement, isValue, jsify, optimize, pure, replace, setJsified, sideEffect, solve, toString, unary, varattr, vari, vop, vop2, _, _ref, _ref1, _ref10, _ref11, _ref12, _ref13, _ref14, _ref15, _ref16, _ref17, _ref18, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
  ...
  exports.optimize = function(exp, env, compiler) {
    ...
    while (1) {
      ...
      switch (task[0]) {
        case TASK_OPTIMIZE:
          result = optimize(task[1], task[2], compiler);
          break;
        case TASK_FUNC:
  ...
  appendTasks = function(compiler, tasks) {
    var compilerTasks, i, _i, _ref19, _results;

    compilerTasks = compiler.tasks;
    _results = [];
    for (i = _i = _ref19 = tasks.length - 1; _ref19 <= 0 ? _i <= 0 : _i >= 0; i = _ref19 <= 0 ? ++_i : --_i) {
      _results.push(compilerTasks.push(tasks[i]));
    }
    return _results;
  };
  ...
  Assign.prototype.optimize = function(env, compiler) {
    var a, left, locals, tasks, _ref19, _ref20, _ref21, _ref22,
      _this = this;
    ...
    if (left instanceof VirtualOperation) {
      left = new left.constructor((function() {
        ...
        for (_i = 0, _len = _ref19.length; _i < _len; _i++) {
          a = _ref19[_i];
          _results.push(optimize(a, env, compiler));
        }
        return _results;
      })());
    } else {
      ...
      if (left instanceof UserLocalVar) {
        locals = (_ref19 = env.userlamda) != null ? _ref19.locals : void 0;
        if (locals) {
          if (locals[left] != null) {
            locals[left]++;
          } else {
        ...
        if ((_ref20 = env.userlamda) != null) {
          _ref20.nonlocals[left] = left;
        }
      } else if (left instanceof InternalLocalVar) {
        if ((_ref21 = env.lamda) != null) {
          _ref21.locals[left] = left;
        }
      } else if (left instanceof InternalNonlocalVar) {
        if ((_ref22 = env.lamda) != null) {
          _ref22.nonlocals[left] = left;
        }
      }
    }
    tasks = [];
    tasks.push([TASK_OPTIMIZE, this.exp, env]);
    tasks.push([
      TASK_FUNC, (function(exp) {
        var assign, envExp, outerEnv;

        assign = new _this.constructor(left, exp);
        outerEnv = env.outer;
        if (outerEnv) {
    ...
    return appendTasks(compiler, tasks);
  };
  ...
  NonlocalDecl = function(env, compiler) {
    var vari, _i, _len, _ref19;
  ...
  If.prototype.optimize = function(env, compiler) {
    ...
    tasks = [];
    tasks.push([TASK_OPTIMIZE, this.test, env]);
    tasks.push([
      TASK_FUNC, (function(test) {
        var test_bool;

        test_bool = boolize(test);
        if (test_bool === true) {
          tasks = [];
          tasks.push([TASK_OPTIMIZE, _this.then_, env]);
          tasks.push([
            TASK_FUNC, (function(then_) {
          ...
          tasks = [];
          tasks.push([TASK_OPTIMIZE, _this.else_, env]);
          tasks.push([
            TASK_FUNC, (function(else_) {
          ...
          tasks.push([
            TASK_FUNC, (function(then_) {
              tasks = [];
              tasks.push([TASK_OPTIMIZE, this.else_, env]);
              tasks.push([
                TASK_FUNC, (function(else_) {
                  if (then_ instanceof If && then_.test === test) {
                    then_ = then_.then_;
  ...
  Throw.prototype.optimize = function(env, compiler) {
    ...
    tasks = [];
    tasks.push([TASK_OPTIMIZE, this.value, env]);
    tasks.push([
      TASK_FUNC, (function(value) {
  ...
  Clamda.prototype.optimize = function(env, compiler) {
    ...
    tasks.push([TASK_OPTIMIZE, this.body, env]);
    tasks.push([
      TASK_FUNC, (function(body) {
        _this.body = jsify(body, compiler, env);
        _this._jsified = true;
        _this._optimized = true;
        return _this;
      })
    ]);
    return appendTasks(compiler, tasks);
  ...
  UserLamda.prototype.optimize = function(env, compiler) {
    var bindings, p, tasks, _i, _len, _ref19,
      _this = this;
  ...
  Lamda.prototype.optimize = function(env, compiler) {
    ...
    env.lamda.childrenLamda.push(this);
    bindings = {};
    _ref19 = this.params;
    for (_i = 0, _len = _ref19.length; _i < _len; _i++) {
      p = _ref19[_i];
  ...
  Apply.prototype.optimize = function(env, compiler) {
    ...
    tasks = [];
    tasks.push([TASK_OPTIMIZE, this.caller, env]);
    tasks.push([
      TASK_FUNC, (function(caller) {
        var a, args, optimizeApply, _i, _len, _ref19;

        tasks = [];
        optimizeApply = caller.optimizeApply;
        if (optimizeApply) {
          return optimizeApply.call(caller, _this.args, env, compiler);
        } else {
          args = [];
          _ref19 = _this.args;
          for (_i = 0, _len = _ref19.length; _i < _len; _i++) {
  ...
  VirtualOperation.prototype.optimize = function(env, compiler) {
    var a, args, tasks, _i, _len, _ref19,
      _this = this;
    ...
    tasks.push([
      TASK_FUNC, (function(args) {
        ...
        if (_isValue && _this.func) {
          return _this.func.apply(null, args);
        } else {
          return new _this.constructor(args);
        }
      })
    ]);
    return appendTasks(compiler, tasks);
  ...
  Begin.prototype.optimize = function(env, compiler) {
    var result, task, tasks, thisExps, waitPop,
      _this = this;
    ...
    tasks.push([
      TASK_FUNC, (function(x) {
        if (result.length > 1) {
          return new _this.constructor(result);
        } else {
          return result[0];
        }
      })
  ...
  JSFun.prototype.optimize = function(env, compiler) {
    ...
    tasks = [];
    tasks.push([TASK_OPTIMIZE, this.fun, env]);
    tasks.push([
      TASK_FUNC, (function(value) {
        return new JSFun(value);
      })
    ]);
    return appendTasks(compiler, tasks);
  };
  ...
  Lamda.prototype.optimizeApply = function(args, env, compiler) {
    ...
    exps.push(this.body);
    tasks.push([TASK_OPTIMIZE, il.topbegin.apply(il, exps), env]);
    tasks.push([
      TASK_FUNC, (function(body) {
        if (!isStatement(body)) {
          return body;
  ...
  Clamda.prototype.optimizeApply = function(args, env, compiler) {
    ...
    return tasks.push([TASK_OPTIMIZE, il.begin(il.assign(this.v, args[0]), this.body), env]);
  };
  ...
  IdCont.prototype.optimizeApply = function(args, env, compiler) {
    return optimize(args[0], env, compiler);
  };

  JSFun.prototype.optimizeApply = function(args, env, compiler) {
    var a, f, optimizedArgs, tasks, _i, _len;
  ...
  CApply.prototype.toCode = function(compiler) {
    return "" + (expressionToCode(compiler, this.caller)) + "(" + (compiler.toCode(this.args[0])) + ")";
  };
  ...
  il.listassign = function() {
    ...
    return il.begin.apply(il, (function() {
      var _j, _ref19, _results;

      _results = [];
      for (i = _j = 0, _ref19 = lefts.length; 0 <= _ref19 ? _j < _ref19 : _j > _ref19; i = 0 <= _ref19 ? ++_j : --_j) {
