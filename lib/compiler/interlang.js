...
(function() {
  ...
  exports.optimize = optimize = function(expr, env, compiler) {
    var a, args, assign, bindings, body, caller, content, e, else_, envExp, exp, exps, f, i, left, locals, outerEnv, p, result, t, test, test_bool, then_, vari, waitPop, _i, _isValue, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref20, _ref21, _ref22, _ref23, _ref24, _ref25, _ref26, _ref27, _ref28;
    ...
    if (expr instanceof Var) {
      ...
      if (left instanceof VirtualOperation) {
        ...
        if (left.isConst) {
          if (left.haveAssigned) {
            throw Error(expr, "should not assign to const more than once.");
          } else {
      ...
      return new Throw(optimize(expr.value, env, compiler));
    } else if (expr instanceof New) {
      return new New(optimize(expr.value, env, compiler));
    } else if (expr instanceof IdCont) {
      return expr;
      ...
      if (caller instanceof IdCont) {
        return optimize(args[0], env);
      } else if (caller instanceof RecursiveClamda) {
        return optimize(il.begin(il.assign(caller.v, args[0]), caller.body), env, compiler);
        ...
        exps = (function() {
          var _len4, _m, _ref28, _results;

          _ref28 = caller.params;
          _results = [];
          for (i = _m = 0, _len4 = _ref28.length; _m < _len4; i = ++_m) {
            p = _ref28[i];
        ...
        body = optimize(il.topbegin.apply(il, exps), env, compiler);
      ...
      if (_isValue && expr.func) {
        return expr.func.apply(null, args);
      } else {
        return new expr.constructor(args);
      }
      ...
      if (result.length > 1) {
        return new expr.constructor(result);
      } else {
        return result[0];
      }
      ...
      if (_.isString(exp)) {
        ...
        return new Deref(optimize(exp, env, compiler));
      }
    } else if (expr instanceof Code) {
      return expr;
    } else if (expr instanceof JSFun) {
      return new JSFun(optimize(expr.fun, env, compiler));
    } else {
      return expr;
    }
  };
