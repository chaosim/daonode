...
(function() {
  var Apply, Array, Assign, Begin, BinaryOperation, BinaryOperationApply, CApply, Clamda, Code, Deref, Element, Fun, If, JSFun, Lamda, MAX_EXTEND_CODE_SIZE, NotImplement, Print, Return, UnaryOperation, UnaryOperationApply, Var, VirtualOperation, VirtualOperationApply, analyse, binary, codeSize, hasOwnProperty, hasSideEffect, il, insertReturn, isEmpty, isStatement, jsify, noSideEffect, occurCount, sideEffect, toString, unary, vop, _, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6,
  ...
  Lamda.prototype.optimize = function(env, compiler) {
    return new Lamda(this.params, compiler.optimize(this.body, env));
  };

  Clamda.prototype.optimize = function(env, compiler) {
    return new Clamda(this.v, compiler.optimize(this.body, env.extend(this.v, this.v)));
  };

  Apply.prototype.optimize = function(env, compiler) {
    var a, arg, args, bindings, body, caller, cont, i, newArgs, newParams, p, params, paramsLength, refCount, refMap, _i, _len;
    ...
    if (caller instanceof JSFun) {
      cont = args[0];
      args = args.slice(1);
      return cont.call(caller.fun.apply(args)).optimize(env, compiler);
      ...
      if (newParams.length === !0) {
        if (!isEmpty(bindings)) {
          env = new Env(env, bindings);
        }
        return new Apply(new Lamda(newParams, compiler.optimze(body, env)), newArgs);
      } else {
        if (bindings) {
          return compiler.optimze(body, new Env(env, bindings));
        } else {
          return compiler.optimize(body, env);
        }
      }
    } else {
      return new this.constructor(caller, args);
    }
  };
  ...
  analyse = function(exp, compiler, refMap) {
    var exp_analyse;

    exp_analyse = exp != null ? exp.analyse : void 0;
    if (exp_analyse) {
      return exp_analyse.call(exp, compiler, refMap);
    }
  };

  Var.prototype.analyse = function(compiler, refMap) {
    if (hasOwnProperty(refMap, this)) {
      return refMap[this]++;
    } else {
      return refMap[this] = 0;
    }
  };

  Assign.prototype.analyse = function(compiler, refMap) {
    return analyse(this.exp, compiler);
  };

  If.prototype.analyse = function(compiler, refMap) {
    return analyse(this.test, compiler, refMap) + analyse(this.then_, compiler, refMap) + analyse(this.else_, compiler, refMap);
  };

  Begin.prototype.analyse = function(compiler, refMap) {
    var e, _i, _len, _ref7, _results;

    _ref7 = this.exps;
    _results = [];
    for (_i = 0, _len = _ref7.length; _i < _len; _i++) {
      e = _ref7[_i];
      _results.push(analyse(e, compiler, refMap));
    }
    return _results;
  };

  Apply.prototype.analyse = function(compiler, refMap) {
    var e, _i, _len, _ref7, _results;

    analyse(this.caller, compiler, refMap);
    _ref7 = this.args;
    _results = [];
    for (_i = 0, _len = _ref7.length; _i < _len; _i++) {
      e = _ref7[_i];
  ...
  CApply.prototype.analyse = function(compiler, refMap) {
    analyse(this.cont, compiler, refMap);
    return analyse(this.value, compiler, refMap);
  };

  Lamda.prototype.analyse = function(compiler, refMap) {
    var childMap, i, x, _results;

    childMap = this.refMap = {};
    analyse(this.body, compiler, childMap);
    _results = [];
    for (x in childMap) {
      i = childMap[x];
      if (__indexOf.call(this.params, x) < 0) {
        if (hasOwnProperty(refMap, x)) {
  ...
  Clamda.prototype.analyse = function(compiler, refMap) {
    var childMap, i, x, _results;
  ...
  CApply.prototype.optimize = function(env, compiler) {
    ...
    body = cont.body;
    v = cont.v;
    value = compiler.optimize(this.value, env);
    count = cont.refMap[v];
    if (sideEffect(value)) {
      switch (count) {
        case 0:
  ...
  occurCount = function(exp, vari) {
    var exp_occurCount;

    exp_occurCount = exp != null ? exp.occurCount : void 0;
    if (exp_occurCount) {
      return exp_occurCount.call(exp, vari);
    } else {
      return 0;
    }
  };

  Var.prototype.occurCount = function(vari) {
    if (this.name === vari.name) {
      return 1;
    } else {
      return 0;
    }
  ...
  Assign.prototype.occurCount = function(vari) {
    return occurCount(this.exp, vari);
  };

  If.prototype.occurCount = function(vari) {
    return occurCount(this.test, vari) + occurCount(this.then_, vari) + occurCount(this.else_, vari);
  };

  Begin.prototype.occurCount = function(vari) {
    return _.reduce(this.exps, (function(memo, e) {
      return memo + occurCount(e, vari);
    }), 0);
  };

  Apply.prototype.occurCount = function(vari) {
    return occurCount(this.caller, vari) + _.reduce(this.args, (function(memo, e) {
      return memo + occurCount(e, vari);
    }), 0);
  };

  CApply.prototype.occurCount = function(vari) {
    return occurCount(this.cont, vari) + occurCount(this.value, vari);
  };

  Lamda.prototype.occurCount = function(vari) {
    if (__indexOf.call(this.params, vari) >= 0) {
      return 0;
    } else {
      return occurCount(this.body, vari);
    }
  };

  Clamda.prototype.occurCount = function(vari) {
    if (vari.name === this.v.name) {
      return 0;
    } else {
      return occurCount(this.body, vari);
    }
  ...
  CApply.prototype.codeSize = function() {
    return codeSize(this.cont.body) + codeSize(this.value) + 2;
  };
  ...
  VirtualOperation.prototype.sideEffect = function() {
    return (this._sideEffects != null) || true;
  };

  BinaryOperation.prototype.sideEffect = function() {
    return (this._sideEffects != null) || false;
  };
  ...
  Apply.prototype.sideEffect = function() {
    var a, caller, _i, _len, _ref7;

    caller = this.caller;
    if (caller instanceof Lamda && sideEffeft(caller.body)) {
      return true;
    }
    if (caller instanceof Clamda && sideEffeft(caller.body)) {
      return true;
    }
  ...
  CApply.prototype.sideEffect = function() {
    return sideEffeft(this.cont.body) || sideEffect(this.value);
  };
  ...
  CApply.prototype.jsify = function() {
    return new CApply(this.cont.jsify(), jsify(this.value));
  };
