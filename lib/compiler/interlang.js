...
(function() {
  var Apply, Array, Assign, Begin, BinaryOperation, BinaryOperationApply, CApply, Clamda, Code, Deref, Element, Env, Fun, IO, If, JSFun, Lamda, MAX_EXTEND_CODE_SIZE, NotImplement, Print, Return, UnaryOperation, UnaryOperationApply, Var, VirtualOperation, VirtualOperationApply, analyze, applyIO, applySideEffect, binary, boolize, codeSize, core, hasOwnProperty, hasSideEffect, il, insertReturn, isEmpty, isStatement, jsify, noSideEffect, optimize, sideEffect, solve, toString, unary, vop, _, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6,
  ...
  Assign.prototype.optimize = function(env, compiler) {
    ...
    return new Assign(left, compiler.optimize(this.exp, env));
  };
  ...
  JSFun.prototype.optimizeApply = function(args, env, compiler) {
    return args[0].call(this.fun.apply(args.slice(1))).optimize(env, compiler);
  };
  ...
  Apply.prototype.sideEffect = function() {
    ...
    if (applySideEffect(this.caller)) {
      return true;
    }
    return false;
  };
  ...
  CApply.prototype.sideEffect = function() {
    return applySideEffect(this.caller) || sideEffect(this.args[0]);
  };
  ...
  Var.prototype.IO = function() {
    return false;
  };

  Return.prototype.IO = function() {
    return IO(this.value);
  };

  If.prototype.IO = function() {
    return sideEffeft(this.test) || IO(this.then_) || IO(this.else_);
  };

  Begin.prototype.IO = function() {
    return _.reduce(this.exps, (function(memo, e) {
      return memo || IO(e);
    }), false);
  };

  Apply.prototype.IO = function() {
    var a, _i, _len, _ref7;
    ...
    if (applyIO(this.caller)) {
      return true;
    }
    return false;
  };

  CApply.prototype.IO = function() {
    return applyIO(this.caller) || IO(this.args[0]);
  };

  applyIO = function(exp) {
    var exp_applyIO;

    exp_applyIO = exp != null ? exp.applyIO : void 0;
    if (exp_applyIO) {
      return exp_applyIO.call(exp);
    } else {
      throw new Error(exp);
    }
  };

  Element.prototype.applyIO = function() {
    throw new NotImplement(this);
  };

  Var.prototype.applyIO = function() {
    return true;
  };

  VirtualOperation.prototype.applyIO = function() {
    if (this._io) {
      return true;
  ...
  BinaryOperation.prototype.applyIO = function() {
    if (this._io) {
      return true;
    } else if (this._io != null) {
      return this._io;
  ...
  UnaryOperation.prototype.applyIO = function() {
    if (this._io) {
      return true;
    } else if (this._io != null) {
      return this._io;
  ...
  JSFun.prototype.applyIO = function() {
    if (this._io) {
      return true;
    } else if (this._io != null) {
      return this._io;
  ...
  Lamda.prototype.applyIO = function() {
    return IO(this.body);
  };

  Clamda.prototype.applyIO = function() {
    return IO(this.body);
  };
