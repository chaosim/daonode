...
(function() {
  ...
  Assign.prototype.jsify = function(compiler, env) {
    ...
    if (exp instanceof Begin) {
      exps = exp.exps;
      length = exps.length;
      exps[length - 1] = new this.constructor(this.left, exps[length - 1]).jsify(compiler, env);
      return exp;
    } else if (exp instanceof If) {
      return new If(exp.test, new this.constructor(this.left, exp.then_), new this.constructor(this.left, exp.else_)).jsify(compiler, env);
    } else if (exp instanceof New) {
      return new this.constructor(this.left, exp);
  ...
  Return.prototype.jsify = function(compiler, env) {
    ...
    exp = jsify(this.value, compiler, env);
    if (exp instanceof Begin) {
      exps = exp.exps;
      length = exps.length;
      exps[length - 1] = new this.constructor(exps[length - 1]).jsify(compiler, env);
      return exp;
    } else if (exp instanceof If) {
      return new If(exp.test, new this.constructor(exp.then_), new this.constructor(exp.else_)).jsify(compiler, env);
    } else if (exp instanceof New) {
      return new this.constructor(exp);
    } else if (exp instanceof Throw) {
      return exp;
    } else if (exp instanceof Return) {
      throw new Error(exp);
  ...
  New.prototype.jsify = function(compiler, env) {
    ...
    if (exp instanceof Begin) {
      ...
      return new If(exp.test, new New(exp.then_), new New(exp.else_)).jsify(compiler, env);
    } else if (exp instanceof New) {
      throw new Error(exp);
    } else if (exp instanceof Throw) {
      return exp;
  ...
  If.prototype.jsify = function(compiler, env) {
    var else_, exps, length, t, test, then_;

    test = this.test, then_ = this.then_, else_ = this.else_;
    ...
    if (test instanceof Begin) {
      ...
      return il.begin(new If(test.test, il.assign(t, test.then_), il.assign(t, test.else_)).jsify(compiler, env), new If(t, then_, else_));
    } else if (test instanceof New) {
      return new If(test, then_, else_);
    } else if (test instanceof Return) {
  ...
  Apply.prototype.jsify = function(compiler, env) {
    var a, aexps, args, caller, caller1, exps, exps1, last, length, t, _i, _len, _ref20;

    exps = [];
    args = [];
    caller = jsify(this.caller, compiler, env);
    if (caller instanceof Begin) {
      exps1 = caller.exps;
      length = exps1.length;
      last = exps1[length - 1];
      if (sideEffect(last) === il.PURE) {
        exps = exps.concat(exps.slice(0, length - 1));
        caller1 = last;
      } else {
      ...
      caller1 = compiler.newvar('caller');
      exps.push(new If(caller.test, il.assign(caller1, caller.then_).jsify(compiler, env), il.assign(caller1, caller.else_).jsify(compiler, env)));
    } else if (caller instanceof New) {
      new Error(caller);
    } else if (caller instanceof Throw) {
      exps.push(caller);
      return il.begin.apply(il, exps);
    } else if (caller instanceof Return) {
    ...
    for (_i = 0, _len = _ref20.length; _i < _len; _i++) {
      ...
      if (a instanceof Begin) {
        ...
        last = aexps[length - 1];
        if (sideEffect(last) === il.PURE) {
          exps = exps.concat(aexps.slice(0, length - 1));
          args.push(last);
        } else {
        ...
        args.push(t);
      } else if (a instanceof New) {
        args.push(a);
      } else if (a instanceof Return) {
        throw new Error(a);
      } else if (a instanceof Throw) {
        exps.push(a);
        return il.begin.apply(il, exps);
      } else {
        args.push(a);
    ...
    exps.push(new this.constructor(caller1, args));
    return il.begin.apply(il, exps);
  };

  CApply.prototype.jsify = function(compiler, env) {
    return new CApply(this.caller.jsify(compiler, env), jsify(this.args[0], compiler, env));
  };

  VirtualOperation.prototype.jsify = function(compiler, env) {
    var a, aexps, args, exps, last, length, t, _i, _len, _ref20;
    ...
    exps.push(new this.constructor(args));
    return il.begin.apply(il, exps);
  };
