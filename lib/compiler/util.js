...
(function() {
  ...
  exports.times = times = function(exp, expectTimes, result, template) {
    ...
    n = internalvar('n');
    if (result == null) {
      return begin(assign(n, 0), any(andp(exp, incp(n))), unify(expectTimes, n));
    } else {
      result1 = internalvar('result');
      return begin(assign(n, 0), assign(result1, []), any(andp(exp, incp(n), pushp(result1, getvalue(template)))), unify(expectTimes, n), unify(result, result1));
    }
  };
  ...
  exports.seplist = function(exp, options) {
    ...
    if (expectTimes === null) {
      ...
      n = internalvar('n');
      if (result === null) {
        return orp(andp(exp, assign(n, 1), any(andp(sep, exp, incp(n))), unify(expectTimes, n)), unify(expectTimes, 0));
      } else {
        return orp(andp(exp, assign(n, 1), assign(result1, list(getvalue(template))), any(andp(sep, exp, pushp(result1, getvalue(template)), incp(n))), unify(expectTimes, n), unify(result, result1)), andp(unify(expectTimes, 0), unify(result, [])));
      }
    }
  };
