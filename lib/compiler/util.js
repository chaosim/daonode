...
(function() {
  var block, break_, continue_, defaultLabel, excludes, if_, iff, il, isLabel, jsfun, lambda, macro, makeLabel, name, not_, quasiquote, string, unquote, unquoteSlice, vop, _, _o,
    __slice = [].slice,
  ...
  exports.iff = iff = function(clauses, else_) {
    ...
    if (length === 0) {
      throw "iff clauses should have at least one clause.";
    } else {
  ...
  exports.print_ = function() {
    ...
    exps = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    return ['funcall', jsfun('console.log')].concat(exps);
  };
  ...
  excludes = ['evalexpr', 'failcont', 'run', 'push', 'getvalue', 'fake', 'popCatch', 'pushCatch', 'findCatch', 'suffixinc', 'suffixdec', 'dec', 'inc'];
