...
(function() {
  var add, assign, begin, block, break_, callcc, catch_, continue_, core, dec, eq, eval_, funcall, if_, iff, inc, jsfun, jsobject, lamda, le, loop_, macall, macro, makeLabel, not_, print_, protect, qq, quote, solve, string, suffixinc, throw_, until_, uq, uqs, vari, while_, xexports, _, _ref;
  ...
  xexports.Test = {
    "test block lamda": function(test) {
      ...
      f = vari('f');
      n = vari('n');
      test.equal(solve(begin(funcall(lamda([x], assign(n, 0), block(a, if_(eq(x, 0), break_(a, n)), assign(n, add(n, dec(x))), continue_(a))), 3), 12)), 12);
      return test.done();
    }
  };
