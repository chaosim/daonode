...
(function() {
  var Error, add, andp, any, assign, begin, boi, bol, char, core, dummy, eoi, eol, eq, findall, funcall, getpos, getstate, gettext, getvalue, greedyany, greedymay, greedysome, if_, inc, jsfun, lamda, lazyany, lazymay, lazysome, lefttext, literal, logicvar, macall, macro, may, nextchar, number, orp, parsetext, print_, quote, settext, solve, some, step, string, subtext, suffixinc, times, vari, xexports, _ref, _ref1;
  ...
  exports.Test = {
    "test times": function(test) {
      ...
      test.deepEqual(solve(begin(assign(_, dummy('__')), assign(n, logicvar('n')), settext(string('ab')), times(char(_), n, result, _), eoi, getvalue(n))), 2);
      ...
      test.deepEqual(solve(begin(settext(string('aaabbb')), assign(n, logicvar('n')), times(char(string('a')), n, result, string('a')), times(char(string('b')), n), eoi, getvalue(n))), 3);
