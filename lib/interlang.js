...
(function() {
  var Apply, Assign, AugmentAssign, Begin, BinaryOperation, BlockLamda, BlockVar, Break, Clamda, Continue, Deref, DoWhile, Element, ExpressionList, For, ForIn, ForOf, Fun, IO, IdCont, If, InternalVar, JSFun, LabelStatement, Lamda, LocalDecl, MAX_EXTEND_CODE_SIZE, New, NonlocalDecl, NotImplement, OptimizableRecuisiveLamda, Print, RecursiveClamda, Return, Switch, Symbol, TailRecuisiveLamda, Throw, Try, UnaryOperation, UserLamda, UserVar, Var, VirtualOperation, While, applySideEffect, augmentAssign, augmentOperators, binary, boolize, codeSize, codeSizeExps, convertBlockLamdaBody, convertOptRecursive, convertOptResursiveCall, convertTailRecursive, convertTailResursiveCall, expressionToCode, expsEffect, getConvertParameters, hasCallOf, hasOwnProperty, il, insertReturn, isArray, isAtomic, isDeclaration, isEmpty, isInteger, isJsified, isObject, isParam, isStatement, isString, isValue, jsify, optimize, pair, postOptimize, pure, replace, setJsified, sideEffect, toString, unary, useConvertParams, util, varattr, vari, vop, vop2, vopMaps, _i, _len, _ref, _ref1, _ref10, _ref11, _ref12, _ref13, _ref14, _ref15, _ref16, _ref17, _ref18, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    __slice = [].slice,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  _ref = require("./util"), isInteger = _ref.isInteger, isString = _ref.isString, isArray = _ref.isArray, isObject = _ref.isObject;
  ...
  Clamda = (function(_super) {
    ...
    Clamda.prototype.call = function(value) {
      return replace(this.body, this.v, value);
    };
  ...
  Try = (function(_super) {
    ...
    function Try(test, vari, catchBody, final) {
      this.test = test;
      this.vari = vari;
    ...
    Try.prototype.toString = function() {
      return "try(" + (toString(this.test)) + ", " + (toString(this.vari)) + ", " + (toString(this.catchBody)) + ", " + (toString(this.final)) + ")";
    };
  ...
  Switch = (function(_super) {
    ...
    function Switch(test, clauses, _default) {
      this.test = test;
    ...
    Switch.prototype.toString = function() {
      return "switch(" + (toString(this.test)) + ", " + (toString(this.clauses)) + ", " + (toString(this["default"])) + ")";
    };
  ...
  Assign.prototype.replace = function(param, value) {
    ...
    if (left instanceof VirtualOperation) {
      left = replace(left, param, value);
    }
    assign = new this.constructor(left, replace(this.exp, param, value));
  ...
  Try.prototype.replace = function(param, value) {
    return new Try(replace(this.test, param, value), this.vari, replace(this.catchBody, param, value), replace(this.final, param, value));
  };

  Switch.prototype.replace = function(param, value) {
    return new Switch(replace(this.test, param, value), replace(this.clauses, param, value), replace(this["default"], param, value));
  };
  ...
  Lamda.prototype.replace = function(param, value) {
    ...
    if (newBody === body) {
      ...
      return new this.constructor(this.params, newBody);
    }
  };
  ...
  Assign.prototype.optimize = function(env, compiler) {
    ...
    } else if (exp instanceof Var && exp.isConst && left.isConst) {
      env.bindings[left] = exp;
      assign._removed = true;
  ...
  Try.prototype.optimize = function(env, compiler) {
    var catchBody, final, test;

    test = optimize(this.test, env, compiler);
    catchBody = optimize(this.catchBody, env, compiler);
    final = optimize(this.final, env, compiler);
    return new Try(test, this.vari, catchBody, final);
  };

  Switch.prototype.optimize = function(env, compiler) {
    return new Switch(optimize(this.test, env, compiler), optimize(this.clauses, env, compiler), optimize(this["default"], env, compiler));
  };
  ...
  Try.prototype.codeSize = function() {
    return codeSize(this.test) + codeSize(this.catchBody) + codeSize(this.final) + 2;
  };

  Switch.prototype.codeSize = function() {
    return codeSize(this.test) + codeSizeExps(this.clauses) + codeSize(this["default"]) + 2;
  };

  Begin.prototype.codeSize = function() {
    return this.exps.reduce((function(memo, e) {
  ...
  Apply.prototype.codeSize = function() {
    return this.args.reduce((function(memo, e) {
      return memo + codeSize(e);
  ...
  Switch.prototype.boolize = function() {
    return void 0;
  };
  ...
  Try.prototype.sideEffect = function() {
    return expsEffect([this.test, this.catchBody, this.final]);
  };

  Switch.prototype.sideEffect = function() {
    return Math.max(expsEffect([this.test, this.catchBody, this.final], sideEffect(this.clauses)));
  };
  ...
  Try.prototype.postOptimize = function(compiler, env) {
    return new Try(postOptimize(this.test, compiler, env), this.vari, postOptimize(this.catchBody, compiler, env), postOptimize(this.final, compiler, env));
  };

  Switch.prototype.postOptimize = function(compiler, env) {
        ...
        _results.push([
          (function() {
            ...
            for (_j = 0, _len1 = _ref19.length; _j < _len1; _j++) {
              id = _ref19[_j];
              _results1.push(postOptimize(id, compiler, env));
            }
            return _results1;
          })(), postOptimize(clause[1], compiler, env)
        ]);
      }
      return _results;
    }).call(this);
    return new Switch(postOptimize(this.test, compiler, env), clauses, postOptimize(this["default"], compiler, env));
  };
  ...
  Try.prototype.jsify = function(compiler, env) {
    return new Try(jsify(this.test, compiler, env), this.vari, jsify(this.catchBody, compiler, env), jsify(this.final, compiler, env));
  };

  Switch.prototype.jsify = function(compiler, env) {
    return new Switch(jsify(this.test, compiler, env), jsify(this.clauses, compiler, env), jsify(this["default"], compiler, env));
  };
  ...
  Try.prototype.convertBlockLamdaBody = function(blockvar) {
    return new Try(convertBlockLamdaBody(this.test, blockvar), this.vari, convertBlockLamdaBody(this.catchBody, blockvar), convertBlockLamdaBody(this.final, blockvar));
  };

  Switch.prototype.convertBlockLamdaBody = function(blockvar) {
    return new Switch(convertBlockLamdaBody(this.test, blockvar), convertBlockLamdaBodyExps(this.clauses, blockvar), convertBlockLamdaBody(this["default"], blockvar));
  };
  ...
  Return.prototype.convertBlockLamdaBody = function(blockvar) {
    ...
    } else if (value instanceof Apply && value.caller === blockvar) {
  ...
  Try.prototype.convertOptRecursive = function(lamda) {
    return new Try(convertOptRecursive(this.test, lamda), this.vari, convertOptRecursive(this.catchBody, lamda), convertOptRecursive(this.final, lamda));
  };

  Switch.prototype.convertOptRecursive = function(lamda) {
    return new Switch(convertOptRecursive(this.test, lamda), convertOptRecursiveExps(this.clauses, lamda), convertOptRecursive(this["default"], lamda));
  };
  ...
  Try.prototype.convertOptResursiveCall = function(lamda, steps) {
    return new Try(convertOptResursiveCall(this.test, lamda, steps), this.vari, convertOptResursiveCall(this.catchBody, lamda, steps), convertOptResursiveCall(this.final, lamda, steps));
  };

  Switch.prototype.convertOptResursiveCall = function(lamda, steps) {
    return new Switch(convertOptResursiveCall(this.test, lamda, steps), convertOptResursiveCall(this.clause, lamda, steps), convertOptResursiveCall(this["default"], lamda, steps));
  };
  ...
  Try.prototype.convertTailRecursive = function(lamda) {
    return new Try(convertTailRecursive(this.test, lamda), this.vari, convertTailRecursive(this.catchBody, lamda), convertTailRecursive(this.final, lamda));
  };

  Switch.prototype.convertTailRecursive = function(lamda) {
    return new Switch(convertTailRecursive(this.test, lamda), convertTailRecursive(this.clauses, lamda), convertTailRecursive(this["default"], lamda));
  };
  ...
  Try.prototype.convertTailResursiveCall = function(lamda, steps) {
    return new Try(convertTailResursiveCall(this.test, lamda, steps), this.vari, convertTailResursiveCall(this.catchBody, lamda, steps), convertTailResursiveCall(this.final, lamda, steps));
  };

  Switch.prototype.convertTailRecursive = function(lamda, steps) {
    return new Switch(convertTailRecursive(this.test, lamda, steps), convertTailRecursiveExps(this.clauses, lamda, steps), convertTailRecursive(this["default"], lamda, steps));
  };
  ...
  Try.prototype.getConvertParameters = function(compiler, lamda) {
    getConvertParameters(this.test, compiler, lamda);
    getConvertParameters(this.catchBody, compiler, lamda);
    return getConvertParameters(this.final, compiler, lamda);
  };

  Switch.prototype.getConvertParameters = function(compiler, lamda) {
    getConvertParameters(this.test, compiler, lamda);
    getConvertParameters(this.clauses, compiler, lamda);
    return getConvertParameters(this["default"], compiler, lamda);
  };
  ...
  Switch.prototype.useConvertParams = function(vars) {
    throw new Error(this);
  };
  ...
  Assign.prototype.hasCallOf = function(vari) {
    return hasCallOf(this.exp, vari);
  };
  ...
  Try.prototype.insertReturn = function() {
    ...
    final = this.final;
    if ((catchBody === null || catchBody === void 0) && (final === null || final === void 0)) {
      final = il["return"](final);
    } else {
      if (catchBody !== null && catchBody !== void 0) {
        catchBody = insertReturn(catchBody);
      }
      if (final !== null && final !== void 0) {
        final = insertReturn(final);
      }
    }
    return new Try(insertReturn(this.test), this.vari, catchBody, final);
  };

  Switch.prototype.insertReturn = function() {
    var clause, clauses;
    ...
    clauses = (function() {
      ...
      for (_i = 0, _len = _ref18.length; _i < _len; _i++) {
        clause = _ref18[_i];
        _results.push([clause[0], insertReturn(clause[1])]);
      }
      return _results;
    }).call(this);
    return new Switch(this.test, clauses, this["default"]);
  };
  ...
  Try.prototype.toCode = function(compiler) {
    ...
    final = this.final;
    if (catchBody === null || catchBody === void 0) {
      return "try{ " + (compiler.toCode(this.test)) + "} finally {" + (compiler.toCode(final)) + "}";
    } else if (final === null || final === void 0) {
      return "try{ " + (compiler.toCode(this.test)) + "} catch (" + this.vari + ") {" + (compiler.toCode(catchBody)) + "}";
    } else {
      return "try{ " + (compiler.toCode(this.test)) + "} catch (" + this.vari + ") {" + (compiler.toCode(catchBody)) + "} finally {" + (compiler.toCode(final)) + "}";
    }
  };

  Switch.prototype.toCode = function(compiler) {
    var body, clause, clauses, default1, e, result, _i, _j, _len, _len1, _ref18;

    result = clauses = this.clauses;
    default1 = this["default"];
    body = '';
    for (_i = 0, _len = clauses.length; _i < _len; _i++) {
      ...
      for (_j = 0, _len1 = _ref18.length; _j < _len1; _j++) {
        e = _ref18[_j];
        body += "case " + (compiler.toCode(e)) + ":";
      }
      body += compiler.toCode(clause[1]) + ';';
    }
    if (default1 === void 0) {
      return "switch (" + (compiler.toCode(this.test)) + "){ " + body + " }";
    } else {
      return "switch (" + (compiler.toCode(this.test)) + "){ " + body + " default: " + (compiler.toCode(default1)) + "}";
    }
  };
  ...
  expressionToCode = function(compiler, exp) {
    if (exp === void 0) {
      return 'exp';
    } else if (!exp.needParenthesis && !(exp instanceof Function)) {
  ...
  Switch.prototype.isStatement = function() {
    return true;
  };
  ...
  il.setobject = function(obj, args) {
    var exps, i, length, _i;

    exps = [];
    length = args.length;
    for (i = _i = 0; _i < length; i = _i += 2) {
      exps.push(il.assign(il.index(obj, args[i]), args[i + 1]));
    }
    return il.begin.apply(il, exps);
  };

  il.uobject = vop('uobject', function(compiler) {
    var args;

    args = this.args;
    return "new UObject(" + (compiler.toCode(args[0])) + ")";
  });
  ...
  il.throwFail = function(value) {
    return il["throw"](il["new"](il.symbol('SolverFail').call(value)));
  };
  ...
  il.setparserdata = function(data) {
    return il.assign(il.parserdata, data);
  };

  il.setparsercursor = function(cursor) {
    return il.assign(il.parsercursor, cursor);
  };
  ...
  il["try"] = function(test, vari, catchBody, final) {
    return new Try(test, vari, catchBody, final);
  };

  il["switch"] = function(test, clauses, default1) {
    return new Switch(test, clauses, default1);
  };
  ...
  _ref18 = [[util.ADD, il.add], [util.SUB, il.sub], [util.MUL, il.mul], [util.DIV, il.div], [util.MOD, il.mod], [util.AND, il.and_], [util.OR, il.or_], [util.NOT, il.not_], [util.BITAND, il.bitand], [util.BITOR, il.bitor], [util.BITXOR, il.bitxor], [util.LSHIFT, il.lshift], [util.RSHIFT, il.rshift], [util.EQ, il.eq], [util.NE, il.ne], [util.LE, il.le], [util.LT, il.lt], [util.GT, il.gt], [util.GE, il.ge], [util.NEG, il.neg], [util.BITNOT, il.bitnot], [util.PUSH, il.push], [util.LIST, il.list], [util.INDEX, il.index], [util.ATTR, il.attr], [util.LENGTH, il.length], [util.SLICE, il.slice], [util.POP, il.pop], [util.INSTANCEOF, il["instanceof"]]];
  for (_i = 0, _len = _ref18.length; _i < _len; _i++) {
    pair = _ref18[_i];
