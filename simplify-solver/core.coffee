...
#when solver succeed at last, @solve is executed.<br/>
...
class exports.Solver
  ...
  fake: () ->
    ...
    result.continues = _.extend({}, @solveinues)
  ...
  solve: (exp, toCont = @done) -> exp?.solve?(@, toCont) or toCont(exp)
  ...
  solveExps: (exps, cont) ->
    ...
    else if length is 1 then @solve(exps[0], cont)
    else @solve(exps[0], (v) => @solveExps(exps[1...], cont))
  ...
  solveArgs: (args, cont) ->
    ...
    switch length
      ...
      when 2
        solver = @
        @solve(args[0], (arg0) -> solver.solve(args[1], (arg1) -> cont([arg0, arg1])))
      when 3
        solver = @
        @solve(args[0], (arg0) ->
          solver.solve(args[1], (arg1) ->
            solver.solve(args[2], (arg2) ->
              cont([arg0, arg1, arg2]))))
      when 4
        solver = @
        @solve(args[0], (arg0) ->
          solver.solve(args[1], (arg1) ->
            solver.solve(args[2], (arg2) ->
              solver.solve(args[3], (arg3) ->
                cont([arg0, arg1, arg2, arg3])))))
      when 5
        solver = @
        @solve(args[0], (arg0) ->
          solver.solve(args[1], (arg1) ->
            solver.solve(args[2], (arg2) ->
              solver.solve(args[3], (arg3) ->
                solver.solve(args[4], (arg4) ->
                  cont([arg0, arg1, arg2, arg3, arg4]))))))
      when 6
        solver = @
        @solve(args[0], (arg0) ->
          solver.solve(args[1], (arg1) ->
            solver.solve(args[2], (arg2) ->
              solver.solve(args[3], (arg3) ->
                solver.solve(args[4], (arg4) ->
                  solver.solve(args[5], (arg5) ->
                    cont([arg0, arg1, arg2, arg3, arg4, arg5])))))))
      when 7
        solver = @
        @solve(args[0], (arg0) ->
          solver.solve(args[1], (arg1) ->
            solver.solve(args[2], (arg2) ->
              solver.solve(args[3], (arg3) ->
                solver.solve(args[4], (arg4) ->
                  solver.solve(args[5], (arg5) ->
                    solver.solve(args[6], (arg6) ->
                      cont([arg0, arg1, arg2, arg3, arg4, arg5, arg6]))))))))
      else
        params = []
        solver = @
        cont = do (cont=cont) -> (argi) -> params.push(argi); cont(params)
        cont = do (cont=cont) -> (argi) -> params.push(argi); solver.solve(args[length-1], cont)
        for i in [length-2..1] by -1
          cont = do (i=i, cont=cont) ->
            (argi) -> (params.push(argi); solver.solve(args[i], cont))
        solver.solve(args[0], cont)
...
Var = class exports.Var
  ...
  solve: (solver, cont) -> cont(@deref(solver.trail))

  # nottodo: variable's solveApply:: canceled. lisp1 should be good.
...
class exports.Apply
  constructor: (@caller, @args) ->
    ...
    # used in macro.solveApply, to prevent maximum recursive depth error

  toString: -> "#{@caller}(#{@args.join(', ')})"

  # get the continuation of an instance of Apply based on cont
  solve: (solver, cont) -> @caller.solveApply(solver, cont, @args)
...
class exports.Special extends exports.Command
  solveApply: (solver, cont, args) -> @fun(solver, cont, args...)

# generate an instance of Special from a function <br/>
#  example:<br/>
#  begin = special(null, 'begin', (solver, cont, exps...) -> solver.solveExps(exps, cont))  # coffeescript <br/>
...
#    return solver.solveExps(exps, cont);<br/>
#  });<br/>
...
# KISS: to keep it simple, not to implmenting the solveApply in Var and Apply<br/>
# call goal with args...
exports.call = special(-1, 'call', (solver, cont, goal, args...) ->
  goal1 = null
  solveArgs =  solver.solveArgs(args, (params,  solver) ->
    solver.cont(goal1(params...), cont)(null))
  solver.cont(goal, (v) -> goal1 = goal; solveArgs(null)))
...
class exports.Fun extends exports.Command
  solveApply: (solver, cont, args) ->  solver.solveArgs(args, (params) => cont(@fun(params...)))
...
class exports.Fun2 extends exports.Command
  solveApply: (solver, cont, args) ->
    fun = @fun
    solver.solveArgs(args, (params) ->
...
class exports.Lamda extends exports.Command
  solveApply: (solver, cont, args) ->
    ...
    solver.solveArgs(args, (params) -> f(cont, params...))
...
#     and specials that don't eval their arguments.
  ...
  special(cmd.arity, name, (solver, cont, args...) ->
          solver.solveArgs(args, (params) -> [solver.cont(cmd(params...), cont), params]))
...
# * **nottodo**: variable's solveApply:: canceled. lisp1 should be good.
# * **nottodo**: Apply's solveApply:: lisp1 should be good.
