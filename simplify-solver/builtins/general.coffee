...
exports.print_ = special(null, 'print_', (solver, cont, args...) ->
  solver.solveArgs(args, (args,  solver) -> console.log(args...); cont(null)))

# x === y 
exports.eq = special(2, 'eq', (solver, cont, x, y) ->
  solver.solve(x, (x1) -> solver.solve(y, (y1) -> cont(x1==y1))))

# x !== y 
exports.ne = special(2, 'ne', (solver, cont, x, y) ->
  ycont =  solver.solve(y, (v) -> cont(x!=v))
  xcont = (v) ->  x = v; ycont(null)
  solver.solve(x, xcont))
...
exports.and_ = special(2, 'and_', (solver, cont, x, y) ->
  ycont =  solver.solve(y, (v) -> cont(x and v))
  xcont = (v) ->  x = v; ycont(null)
  solver.solve(x, xcont))
...
# ~x 
exports.bitnot = special(1, 'bitnot', (solver, cont, x) ->
  solver.solve(x, (v) -> cont(~v)))
...
exports.inc2 = special(1, 'inc2', (solver, cont, vari) ->
  (vari.binding++; cont(++vari.binding)))
...
exports.getvalue = special(1, 'getvalue', (solver, cont, x) ->
  solver.solve(x, (v) -> cont(solver.trail.getvalue(v))))

# x.length 
exports.length = special(1, 'length', (solver, cont, x) ->
  solver.solve(x, (v) -> cont(v.length)))
...
exports.third = special(1, 'third', (solver, cont, x) ->
  solver.solve(x, (v) -> cont(v[2])))

# x.concat(y) 
exports.concat = special(2, 'concat', (solver, cont, x, y) ->
  ycont =  solver.solve(y, (v) -> cont(x.concat(y)))
  xcont = (v) ->  x = v; ycont(null)
  solver.solve(x, xcont))
...
exports.toString = special(1, 'toString', (solver, cont, x) ->
  solver.solve(x, (v) -> cont(v?.toString?() or JSON.stringify(v))))
