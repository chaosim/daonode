...
(function() {
  var compileToJSFile, cons, dao, _ref,
  ...
  dao.Cons = (function() {
    ...
    Cons.prototype.copy = function(memo) {
      return new Cons(copy(this.head, memo), copy(this.tail, memo));
    };
  ...
  dao["eval"] = function(exp, env) {
    return dao.solve(exp, env).next();
  };

  compileToJSFile = require('../src/compile').compileToJSFile;

  dao.solve = function(exp, env, path) {
    var compiled;

    path = path || 'f:\\daonode\\test\\compiled.js';
    compileToJSFile(exp, env, path);
    compiled = require(path);
    return new dao.Solutions(exp, compiled.fun());
  };
