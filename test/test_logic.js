...
(function() {
  var add, andp, array, assign, begin, cons, core, cut, cutable, eq, fail, findall, funcall, getvalue, if_, inc, lamda, le, logicvar, macro, makeobject, not_, notp, once, orp, orp2, print_, solve, solvebase, string, succeed, uarray, unify, uobject, vari, xexports, _, _ref;
  ...
  _ref = require('../lib/util'), begin = _ref.begin, assign = _ref.assign, print_ = _ref.print_, string = _ref.string, array = _ref.array, uarray = _ref.uarray, cons = _ref.cons, makeobject = _ref.makeobject, uobject = _ref.uobject, funcall = _ref.funcall, lamda = _ref.lamda, macro = _ref.macro, if_ = _ref.if_, add = _ref.add, eq = _ref.eq, le = _ref.le, inc = _ref.inc, not_ = _ref.not_, logicvar = _ref.logicvar, unify = _ref.unify, succeed = _ref.succeed, fail = _ref.fail, andp = _ref.andp, orp = _ref.orp, orp2 = _ref.orp2, notp = _ref.notp, getvalue = _ref.getvalue, cutable = _ref.cutable, cut = _ref.cut, findall = _ref.findall, once = _ref.once;
  ...
  exports.Test = {
    ...
    "test findall once": function(test) {
      ...
      test.deepEqual(solve(andp(assign(x, logicvar('x')), assign(result, logicvar('result')), findall(orp2(unify(x, 1), unify(x, 2)), result, x), getvalue(result))), [1, 2]);
    ...
    "test unify cons": function(test) {
      ...
      test.equal(solve(begin(assign(a, $a), orp2(andp(unify(cons(a, null), cons(1, null)), unify(a, 2)), unify(a, 2)))), true);
      return test.done();
    },
    "test unify uobject": function(test) {
      ...
      a = vari('a');
      $a = logicvar('a');
      test.equal(solve(unify(makeobject(string('a'), 1), {
        a: 1
      })), false);
      test.equal(solve(unify(uobject(string('a'), 1), {
        a: 1
      })), true);
      test.equal(solve(unify(uobject(string('a'), $a), {
        a: 1
      })), true);
      test.equal(solve(andp(assign(a, $a), unify(uobject(string('a'), a), {
        a: 1
      }), unify(a, 2))), false);
      test.equal(solve(begin(assign(a, $a), orp2(andp(unify(uobject(string('a'), a), {
        a: 1
