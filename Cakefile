# todo: this is copied from Jeremy Ashkenas' coffeescript cakefile.
# I'm working on porting it for daonode, and not finished.
...
header = """
  /**
   * daonode v#{daonode.VERSION}
   * https://github.com/chaosim/daonode
   *
   * Copyright 2011, 曹星明 Caoxingming Simeon.chaos@gmail.com
   * Released under the MIT License
   */
"""

# Build the dao language from source.
build = (cb) ->
  files = fs.readdirSync 'lib'
  files = ('lib/' + file for file in files when file.match(/\.(lit)?coffee$/))
  run ['-c', '-o', 'lib/coffee-script'].concat(files), cb

# Run a daonode through our node/daonode interpreter.
run = (args, cb) ->
...
task 'install', 'install Daonode into /usr/local (or --prefix)', (options) ->
...
task 'build', 'build the Dao language from source', build
...
task 'test', 'run the daonode suite', ->
  runTests CoffeeScript
